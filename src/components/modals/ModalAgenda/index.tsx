...
import DatePickerStyled from '@/components/selectDateStyled'
...
const ModalAgenda = ({
  ...
  const patientSearch = useMemo(() => {
    return data.find((option) => option.id.toString() === formik.values.patient.toString()) || null
                ...
                {!viewTwo && (
                  ...
                  componentsProps={{
                    ...
                    {(formik.errors.patient && formik.touched.patient) && (
                      <p className='font-light text-red text-sm pt-1 text-center'>{formik.errors.patient}</p>
                    )}

...
const HeaderDash = () => {
  ...
  return (
    <div className="flex border-b border-b-gray justify-end items-center px-4 py-2 relative bg-white dark:bg-gray-800  dark:border-gray-700 dark:border-b shadow-lg">
      <button onClick={toggleTheme} className='flex gap-2 p-2 border rounded-6 dark:bg-gray-700 dark:border-slate-600 items-center justify-center'>
        {themeDark ? (
        ...
        <p className='dark:text-white text-sm font-light'>{themeDark ? 'Escuro' : 'Claro'}</p>
      </button>
    </div>
  )
}
